"use client";

import { useEffect, useState } from "react";
import { supabase } from "../../../lib/supabase";
import { gerarPDFPadrao } from "../../../lib/relatoriopdf";

type Venda = {
  id: string;
  status: string | null;
  created_at: string | null;
};

type ItemVenda = {
  id: string;
  venda_id: string;
  produto_id: string;
  quantidade: number | null;
  valor_unitario: number | null;
  subtotal: number | null;
};

type Produto = {
  id: string;
  nome: string;
  codigo: string | null;
  unidade: string | null;
  preco_custo: number | null;
  preco_venda: number | null;
};

type ProdutoLucro = {
  produto_id: string;
  codigo: string;
  nome: string;
  unidade: string;
  quantidade: number;
  receita: number;
  custo: number;
  lucro: number;
  margem: number;
  preco_custo: number;
  preco_venda: number;
};

export default function RelatorioLucratividadePage() {
  const [dataInicial, setDataInicial] = useState("");
  const [dataFinal, setDataFinal] = useState("");
  const [ordenacao, setOrdenacao] = useState("lucro");

  const [empresaNome, setEmpresaNome] = useState("Empresa");
  const [usuarioNome, setUsuarioNome] = useState("Administrador");

  const [produtosLucro, setProdutosLucro] = useState<ProdutoLucro[]>([]);
  const [totalProdutos, setTotalProdutos] = useState(0);
  const [quantidadeVendida, setQuantidadeVendida] = useState(0);
  const [receitaTotal, setReceitaTotal] = useState(0);
  const [custoTotal, setCustoTotal] = useState(0);
  const [lucroTotal, setLucroTotal] = useState(0);
  const [margemMedia, setMargemMedia] = useState(0);

  function moeda(valor: number) {
    return Number(valor || 0).toLocaleString("pt-BR", {
      style: "currency",
      currency: "BRL",
    });
  }

  function numero(valor: number) {
    return Number(valor || 0).toLocaleString("pt-BR");
  }

  function hojeISO() {
    return new Date().toISOString().split("T")[0];
  }

  function primeiroDiaMesISO() {
    const hoje = new Date();

    return new Date(hoje.getFullYear(), hoje.getMonth(), 1)
      .toISOString()
      .split("T")[0];
  }

  function formatarDataBR(data: string) {
    if (!data) return "-";
    return new Date(data + "T00:00:00").toLocaleDateString("pt-BR");
  }

  function prepararDatas() {
    const inicio = dataInicial || primeiroDiaMesISO();
    const fim = dataFinal || hojeISO();

    const inicioISO = new Date(inicio + "T00:00:00").toISOString();
    const fimISO = new Date(fim + "T23:59:59").toISOString();

    return {
      inicio,
      fim,
      inicioISO,
      fimISO,
    };
  }

  function classeMargem(margem: number) {
    if (margem >= 40) return "text-green-700";
    if (margem >= 20) return "text-blue-700";
    if (margem >= 10) return "text-orange-700";
    return "text-red-700";
  }

  function statusMargem(margem: number) {
    if (margem >= 40) {
      return {
        texto: "Excelente",
        classe: "bg-green-100 text-green-700",
      };
    }

    if (margem >= 20) {
      return {
        texto: "Boa",
        classe: "bg-blue-100 text-blue-700",
      };
    }

    if (margem >= 10) {
      return {
        texto: "Atenção",
        classe: "bg-orange-100 text-orange-700",
      };
    }

    return {
      texto: "Baixa",
      classe: "bg-red-100 text-red-700",
    };
  }

  function zerarResumo() {
    setProdutosLucro([]);
    setTotalProdutos(0);
    setQuantidadeVendida(0);
    setReceitaTotal(0);
    setCustoTotal(0);
    setLucroTotal(0);
    setMargemMedia(0);
  }

  async function carregarRelatorio() {
    const usuario = JSON.parse(localStorage.getItem("th_usuario") || "{}");
    const empresaId = usuario.empresa_id;

    setEmpresaNome(usuario.empresa_nome || "Empresa");
    setUsuarioNome(usuario.nome || "Administrador");

    const { inicio, fim, inicioISO, fimISO } = prepararDatas();

    if (!dataInicial) setDataInicial(inicio);
    if (!dataFinal) setDataFinal(fim);

    if (!empresaId) {
      zerarResumo();
      return;
    }

    const vendasReq = await supabase
      .from("vendas")
      .select("id,status,created_at")
      .eq("empresa_id", empresaId)
      .eq("status", "finalizada")
      .gte("created_at", inicioISO)
      .lte("created_at", fimISO);

    if (vendasReq.error) {
      alert("Erro ao carregar vendas: " + vendasReq.error.message);
      return;
    }

    const vendas: Venda[] = vendasReq.data || [];
    const vendaIds = vendas.map((venda) => venda.id);

    if (vendaIds.length === 0) {
      zerarResumo();
      return;
    }

    const itensReq = await supabase
      .from("itens_venda")
      .select("id,venda_id,produto_id,quantidade,valor_unitario,subtotal")
      .in("venda_id", vendaIds);

    if (itensReq.error) {
      alert("Erro ao carregar itens de venda: " + itensReq.error.message);
      return;
    }

    const itens: ItemVenda[] = itensReq.data || [];

    const produtosReq = await supabase
      .from("produtos")
      .select("id,nome,codigo,unidade,preco_custo,preco_venda")
      .eq("empresa_id", empresaId)
      .order("nome", { ascending: true });

    if (produtosReq.error) {
      alert("Erro ao carregar produtos: " + produtosReq.error.message);
      return;
    }

    const produtos: Produto[] = produtosReq.data || [];
    const produtoIdsDaEmpresa = new Set(produtos.map((produto) => produto.id));

    const itensDaEmpresa = itens.filter((item) =>
      produtoIdsDaEmpresa.has(item.produto_id)
    );

    const mapa = new Map<
      string,
      {
        quantidade: number;
        receita: number;
        custo: number;
      }
    >();

    itensDaEmpresa.forEach((item) => {
      const produto = produtos.find((prod) => prod.id === item.produto_id);

      const quantidade = Number(item.quantidade || 0);
      const receita = Number(item.subtotal || 0);
      const custoUnitario = Number(produto?.preco_custo || 0);
      const custo = quantidade * custoUnitario;

      const atual = mapa.get(item.produto_id) || {
        quantidade: 0,
        receita: 0,
        custo: 0,
      };

      atual.quantidade += quantidade;
      atual.receita += receita;
      atual.custo += custo;

      mapa.set(item.produto_id, atual);
    });

    let lista: ProdutoLucro[] = Array.from(mapa.entries()).map(
      ([produto_id, dados]) => {
        const produto = produtos.find((prod) => prod.id === produto_id);

        const lucro = dados.receita - dados.custo;
        const margem = dados.receita > 0 ? (lucro / dados.receita) * 100 : 0;

        return {
          produto_id,
          codigo: produto?.codigo || "-",
          nome: produto?.nome || "Produto não encontrado",
          unidade: produto?.unidade || "-",
          quantidade: dados.quantidade,
          receita: dados.receita,
          custo: dados.custo,
          lucro,
          margem,
          preco_custo: Number(produto?.preco_custo || 0),
          preco_venda: Number(produto?.preco_venda || 0),
        };
      }
    );

    if (ordenacao === "lucro") {
      lista.sort((a, b) => b.lucro - a.lucro);
    }

    if (ordenacao === "margem") {
      lista.sort((a, b) => b.margem - a.margem);
    }

    if (ordenacao === "receita") {
      lista.sort((a, b) => b.receita - a.receita);
    }

    if (ordenacao === "quantidade") {
      lista.sort((a, b) => b.quantidade - a.quantidade);
    }

    if (ordenacao === "menor_margem") {
      lista.sort((a, b) => a.margem - b.margem);
    }

    const receita = lista.reduce(
      (total, item) => total + Number(item.receita || 0),
      0
    );

    const custo = lista.reduce(
      (total, item) => total + Number(item.custo || 0),
      0
    );

    const lucro = receita - custo;

    const qtd = lista.reduce(
      (total, item) => total + Number(item.quantidade || 0),
      0
    );

    setProdutosLucro(lista);
    setTotalProdutos(lista.length);
    setQuantidadeVendida(qtd);
    setReceitaTotal(receita);
    setCustoTotal(custo);
    setLucroTotal(lucro);
    setMargemMedia(receita > 0 ? (lucro / receita) * 100 : 0);
  }

  async function imprimirPdf() {
    await gerarPDFPadrao(
      "LUCRATIVIDADE",
      ["Código", "Produto", "Un", "Qtd", "Receita", "Custo", "Lucro", "Margem"],
      produtosLucro.map((item) => [
        item.codigo,
        item.nome,
        item.unidade,
        numero(item.quantidade),
        moeda(item.receita),
        moeda(item.custo),
        moeda(item.lucro),
        `${item.margem.toFixed(2)}%`,
      ])
    );
  }

  useEffect(() => {
    const inicio = primeiroDiaMesISO();
    const fim = hojeISO();

    setDataInicial(inicio);
    setDataFinal(fim);
  }, []);

  useEffect(() => {
    if (dataInicial && dataFinal) {
      carregarRelatorio();
    }
  }, [dataInicial, dataFinal, ordenacao]);

  return (
    <div className="bg-slate-50 min-h-screen p-8 print:bg-white print:p-0">
      <style jsx global>{`
        @media print {
          @page {
            size: A4;
            margin: 10mm;
          }

          * {
            -webkit-print-color-adjust: exact !important;
            print-color-adjust: exact !important;
          }

          body {
            background: white !important;
          }

          aside,
          header,
          .no-print {
            display: none !important;
          }

          main {
            min-height: auto !important;
          }

          .documento-relatorio {
            width: 100% !important;
            max-width: 100% !important;
            margin: 0 !important;
            box-shadow: none !important;
            border: none !important;
            padding: 0 !important;
          }

          .quebra-pagina {
            page-break-inside: avoid;
          }
        }
      `}</style>

      <div className="no-print max-w-5xl mx-auto mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900">
            Relatório de Lucratividade
          </h1>

          <p className="text-slate-500">
            Receita, custo, lucro bruto e margem por produto vendido.
          </p>
        </div>

        <button
          onClick={imprimirPdf}
          className="bg-blue-700 hover:bg-blue-800 text-white px-6 py-3 rounded-2xl font-bold shadow-sm"
        >
          Imprimir / Salvar PDF
        </button>
      </div>

      <div className="no-print max-w-5xl mx-auto bg-white rounded-3xl border border-slate-200 shadow-sm p-6 mb-6">
        <h2 className="text-xl font-black text-slate-900 mb-4">
          Filtros
        </h2>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-sm font-semibold text-slate-600 mb-2">
              Data Inicial
            </label>

            <input
              type="date"
              value={dataInicial}
              onChange={(e) => setDataInicial(e.target.value)}
              className="w-full border border-slate-300 p-3 rounded-xl text-slate-900"
            />
          </div>

          <div>
            <label className="block text-sm font-semibold text-slate-600 mb-2">
              Data Final
            </label>

            <input
              type="date"
              value={dataFinal}
              onChange={(e) => setDataFinal(e.target.value)}
              className="w-full border border-slate-300 p-3 rounded-xl text-slate-900"
            />
          </div>

          <div>
            <label className="block text-sm font-semibold text-slate-600 mb-2">
              Ordenar por
            </label>

            <select
              value={ordenacao}
              onChange={(e) => setOrdenacao(e.target.value)}
              className="w-full border border-slate-300 p-3 rounded-xl text-slate-900 bg-white"
            >
              <option value="lucro">Maior Lucro</option>
              <option value="margem">Maior Margem</option>
              <option value="menor_margem">Menor Margem</option>
              <option value="receita">Maior Receita</option>
              <option value="quantidade">Maior Quantidade</option>
            </select>
          </div>

          <div className="flex items-end">
            <button
              onClick={carregarRelatorio}
              className="w-full bg-slate-800 hover:bg-slate-900 text-white px-6 py-3 rounded-xl font-bold"
            >
              Atualizar
            </button>
          </div>
        </div>
      </div>

      <div className="documento-relatorio max-w-5xl mx-auto bg-white border border-slate-200 shadow-lg rounded-2xl overflow-hidden">
        <div className="px-8 pt-8 pb-5">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5 border-b-4 border-blue-700 pb-5">
            <div className="flex items-center gap-4">
              <img
                src="/logo-thcloud-transparente.png"
                alt="THCloud"
                className="h-14 w-14 object-contain"
                onError={(e) => {
                  e.currentTarget.src = "/logo-thcloud.jpeg";
                }}
              />

              <div>
                <h2 className="text-2xl font-black text-blue-800">
                  THCloud
                </h2>

                <p className="text-sm text-slate-500 font-semibold">
                  Gestão Inteligente
                </p>
              </div>
            </div>

            <div className="text-center">
              <h1 className="text-2xl md:text-3xl font-black text-slate-900">
                LUCRATIVIDADE
              </h1>

              <p className="text-sm text-slate-500 font-semibold mt-1">
                Relatório Gerencial
              </p>
            </div>

            <div className="text-right text-xs text-slate-600">
              <p>
                <strong>Período:</strong> {formatarDataBR(dataInicial)} até{" "}
                {formatarDataBR(dataFinal)}
              </p>

              <p className="mt-1">
                <strong>Emissão:</strong>{" "}
                {new Date().toLocaleString("pt-BR")}
              </p>
            </div>
          </div>
        </div>

        <div className="px-8 pb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="border border-slate-200 rounded-xl overflow-hidden">
              <div className="bg-slate-100 px-4 py-3 text-blue-800 font-black text-sm border-b border-slate-200">
                DADOS DA EMPRESA
              </div>

              <table className="w-full text-sm">
                <tbody>
                  <LinhaInfo titulo="Empresa" valor={empresaNome} />
                  <LinhaInfo titulo="Sistema" valor="Th Cloud" />
                  <LinhaInfo titulo="Site" valor="thcloud.com.br" />
                  <LinhaInfo titulo="Responsável" valor={usuarioNome} />
                </tbody>
              </table>
            </div>

            <div className="border border-slate-200 rounded-xl overflow-hidden">
              <div className="bg-slate-100 px-4 py-3 text-blue-800 font-black text-sm border-b border-slate-200">
                DADOS DO RELATÓRIO
              </div>

              <table className="w-full text-sm">
                <tbody>
                  <LinhaInfo titulo="Relatório" valor="Lucratividade" />
                  <LinhaInfo
                    titulo="Período"
                    valor={`${formatarDataBR(dataInicial)} até ${formatarDataBR(
                      dataFinal
                    )}`}
                  />
                  <LinhaInfo titulo="Ordenação" valor={ordenacao} />
                  <LinhaInfo titulo="Produtos" valor={`${totalProdutos}`} />
                </tbody>
              </table>
            </div>
          </div>
        </div>

        <div className="px-8 pb-6 no-print">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <ResumoCard titulo="Receita" valor={moeda(receitaTotal)} cor="text-green-700" />
            <ResumoCard titulo="Custo" valor={moeda(custoTotal)} cor="text-red-700" />
            <ResumoCard titulo="Lucro Bruto" valor={moeda(lucroTotal)} cor={lucroTotal >= 0 ? "text-blue-700" : "text-red-700"} />
            <ResumoCard titulo="Margem" valor={`${margemMedia.toFixed(2)}%`} cor={classeMargem(margemMedia)} />
          </div>
        </div>

        <div className="px-8 pb-6 quebra-pagina">
          <h2 className="text-xl font-black text-slate-900 mb-4">
            Resumo da Lucratividade
          </h2>

          <table className="w-full border border-slate-300 text-sm">
            <thead>
              <tr className="bg-blue-950 text-white">
                <th className="p-3 text-left">DESCRIÇÃO</th>
                <th className="p-3 text-right w-52">TOTAL</th>
              </tr>
            </thead>

            <tbody>
              <LinhaResumo titulo="RECEITA TOTAL" detalhe="Soma dos subtotais das vendas" valor={moeda(receitaTotal)} cor="text-green-700" />
              <LinhaResumo titulo="CUSTO TOTAL" detalhe="Quantidade vendida x preço de custo" valor={moeda(custoTotal)} cor="text-red-700" />
              <LinhaResumo titulo="LUCRO BRUTO" detalhe="Receita total - custo total" valor={moeda(lucroTotal)} cor={lucroTotal >= 0 ? "text-blue-700" : "text-red-700"} />
              <LinhaResumo titulo="MARGEM MÉDIA" detalhe="Lucro dividido pela receita" valor={`${margemMedia.toFixed(2)}%`} cor={classeMargem(margemMedia)} />
              <LinhaResumo titulo="QUANTIDADE VENDIDA" detalhe="Soma das unidades vendidas" valor={numero(quantidadeVendida)} cor="text-purple-700" />
              <LinhaResumo titulo="PRODUTOS VENDIDOS" detalhe="Produtos diferentes vendidos no período" valor={`${totalProdutos}`} cor="text-blue-950" />
            </tbody>
          </table>
        </div>

        <div className="px-8 pb-6 quebra-pagina">
          <h2 className="text-xl font-black text-slate-900 mb-4">
            Lucratividade por Produto
          </h2>

          <table className="w-full border border-slate-300 text-xs">
            <thead>
              <tr className="bg-blue-950 text-white">
                <th className="p-3 text-left">PRODUTO</th>
                <th className="p-3 text-left">UN</th>
                <th className="p-3 text-right">QTD</th>
                <th className="p-3 text-right">RECEITA</th>
                <th className="p-3 text-right">CUSTO</th>
                <th className="p-3 text-right">LUCRO</th>
                <th className="p-3 text-right">MARGEM</th>
                <th className="p-3 text-left">STATUS</th>
              </tr>
            </thead>

            <tbody>
              {produtosLucro.map((item, index) => {
                const status = statusMargem(item.margem);

                return (
                  <tr
                    key={item.produto_id}
                    className={`${index % 2 === 0 ? "bg-white" : "bg-slate-50"} border-b border-slate-200`}
                  >
                    <td className="p-3">
                      <p className="font-semibold text-slate-900">{item.nome}</p>
                      <p className="text-slate-500">Código: {item.codigo}</p>
                    </td>

                    <td className="p-3 text-slate-700">
                      {item.unidade}
                    </td>

                    <td className="p-3 text-right font-black text-slate-900">
                      {numero(item.quantidade)}
                    </td>

                    <td className="p-3 text-right font-black text-green-700">
                      {moeda(item.receita)}
                    </td>

                    <td className="p-3 text-right font-black text-red-700">
                      {moeda(item.custo)}
                    </td>

                    <td
                      className={`p-3 text-right font-black ${
                        item.lucro >= 0 ? "text-blue-700" : "text-red-700"
                      }`}
                    >
                      {moeda(item.lucro)}
                    </td>

                    <td className={`p-3 text-right font-black ${classeMargem(item.margem)}`}>
                      {item.margem.toFixed(2)}%
                    </td>

                    <td className="p-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold ${status.classe}`}>
                        {status.texto}
                      </span>
                    </td>
                  </tr>
                );
              })}

              {produtosLucro.length === 0 && (
                <tr>
                  <td colSpan={8} className="p-6 text-center text-slate-500">
                    Nenhuma venda encontrada no período selecionado para esta empresa.
                  </td>
                </tr>
              )}
            </tbody>

            {produtosLucro.length > 0 && (
              <tfoot>
                <tr className="bg-slate-100 border-t border-slate-300">
                  <td colSpan={2} className="p-3 text-right font-black text-slate-900">
                    TOTAIS
                  </td>

                  <td className="p-3 text-right font-black text-slate-900">
                    {numero(quantidadeVendida)}
                  </td>

                  <td className="p-3 text-right font-black text-green-700">
                    {moeda(receitaTotal)}
                  </td>

                  <td className="p-3 text-right font-black text-red-700">
                    {moeda(custoTotal)}
                  </td>

                  <td className="p-3 text-right font-black text-blue-700">
                    {moeda(lucroTotal)}
                  </td>

                  <td className={`p-3 text-right font-black ${classeMargem(margemMedia)}`}>
                    {margemMedia.toFixed(2)}%
                  </td>

                  <td></td>
                </tr>
              </tfoot>
            )}
          </table>
        </div>

        <div className="px-8 pb-6">
          <div className="border border-blue-300 rounded-xl p-4 flex gap-3">
            <div className="w-7 h-7 rounded-full bg-blue-700 text-white flex items-center justify-center font-black text-sm">
              i
            </div>

            <div>
              <p className="font-black text-blue-800">Observação</p>

              <p className="text-sm text-slate-700 mt-1">
                Relatório gerado automaticamente pelo Th Cloud. O custo é calculado
                multiplicando a quantidade vendida pelo preço de custo atual do cadastro
                do produto da empresa logada. A receita usa o subtotal registrado no item
                da venda.
              </p>
            </div>
          </div>
        </div>

        <div className="px-8 py-5 border-t border-slate-300 bg-white">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center text-xs text-slate-700">
            <div>
              <p className="font-bold text-slate-900">
                Th Cloud - Sistema de Gestão
              </p>
            </div>

            <div className="text-center">
              <p className="font-bold text-blue-700">www.thcloud.com.br</p>
            </div>

            <div className="text-right">
              <p>Página 1 de 1</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function LinhaInfo({ titulo, valor }: { titulo: string; valor: string }) {
  return (
    <tr className="border-b last:border-b-0 border-slate-100">
      <td className="bg-slate-50 p-3 font-bold text-slate-800 w-32">
        {titulo}:
      </td>
      <td className="p-3 text-slate-800 font-medium">{valor}</td>
    </tr>
  );
}

function LinhaResumo({
  titulo,
  detalhe,
  valor,
  cor,
}: {
  titulo: string;
  detalhe: string;
  valor: string;
  cor: string;
}) {
  return (
    <tr className="border-b border-slate-200">
      <td className="p-4">
        <p className="font-black text-slate-900">{titulo}</p>
        <p className="text-xs text-slate-500">{detalhe}</p>
      </td>

      <td className={`p-4 text-right font-black ${cor}`}>
        {valor}
      </td>
    </tr>
  );
}

function ResumoCard({
  titulo,
  valor,
  cor,
}: {
  titulo: string;
  valor: string;
  cor: string;
}) {
  return (
    <div className="border border-slate-200 rounded-xl p-4 bg-white shadow-sm">
      <p className="text-sm font-semibold text-slate-600">{titulo}</p>
      <p className={`text-2xl font-black mt-2 ${cor}`}>{valor}</p>
    </div>
  );
}
