...
export default function RelatorioDREPage() {
  ...
  const [margemResultado, setMargemResultado] = useState(0);
  ...
  async function carregarRelatorio() {
    ...
    const vendasReq = await supabase
      .from("vendas")
      .select("id,valor_total,desconto,status,created_at")
    ...
    const despesasReq = await supabase
      .from("contas_pagar")
    ...
    if (despesasReq.error) {
      alert("Erro ao carregar despesas: " + despesasReq.error.message);
      return;
    }

    const listaVendas: Venda[] = vendasReq.data || [];
    const listaDespesas: ContaPagar[] = despesasReq.data || [];

    const totalReceita = listaVendas.reduce(
      (total, venda) => total + Number(venda.valor_total || 0),
      0
    );

    const totalDescontos = listaVendas.reduce(
    ...
    const liquida = totalReceita - totalDescontos;
    const res = liquida - totalDespesas;

    setVendas(listaVendas);
    setDespesas(listaDespesas);
    setReceitaBruta(totalReceita);
    setDescontos(totalDescontos);
    setReceitaLiquida(liquida);
    setDespesasPagas(totalDespesas);
    setResultado(res);
    setMargemResultado(liquida > 0 ? (res / liquida) * 100 : 0);
    setQuantidadeVendas(listaVendas.length);
  ...
  return (
    <div className="bg-slate-50 min-h-screen p-8 print:bg-white print:p-0">
      ...
      <div className="no-print max-w-5xl mx-auto mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900">DRE</h1>
          <p className="text-slate-500">
            Demonstrativo de Resultado com receitas, despesas e resultado líquido.
          </p>
        </div>
      ...
      <div className="documento-relatorio max-w-5xl mx-auto bg-white border border-slate-200 shadow-lg rounded-2xl overflow-hidden">
        ...
        <div className="px-8 pb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <BoxInfo titulo="DADOS DO RELATÓRIO">
              <LinhaInfo titulo="Relatório" valor="DRE" />
              <LinhaInfo
                titulo="Período"
              ...
              <LinhaInfo titulo="Despesas" valor={`${quantidadeDespesas}`} />
            </BoxInfo>
          </div>
        </div>
        ...
        <div className="px-8 pb-6 no-print">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <ResumoCard titulo="Receita Bruta" valor={moeda(receitaBruta)} cor="text-green-700" />
            <ResumoCard titulo="Descontos" valor={moeda(descontos)} cor="text-red-700" />
            <ResumoCard titulo="Despesas" valor={moeda(despesasPagas)} cor="text-orange-700" />
            <ResumoCard titulo="Resultado" valor={moeda(resultado)} cor={resultado >= 0 ? "text-blue-700" : "text-red-700"} />
          </div>
        </div>
        ...
        <div className="px-8 pb-6">
          ...
          <table className="w-full border border-slate-300 text-sm">
            ...
            <tbody>
              <LinhaResumo titulo="RECEITA BRUTA" detalhe="Total de vendas finalizadas" valor={moeda(receitaBruta)} cor="text-green-700" />
              <LinhaResumo titulo="(-) DESCONTOS" detalhe="Descontos concedidos nas vendas" valor={moeda(descontos)} cor="text-red-700" />
              <LinhaResumo titulo="RECEITA LÍQUIDA" detalhe="Receita bruta menos descontos" valor={moeda(receitaLiquida)} cor="text-blue-700" />
              <LinhaResumo titulo="(-) DESPESAS PAGAS" detalhe="Contas a pagar com status pago" valor={moeda(despesasPagas)} cor="text-orange-700" />
              <LinhaResumo titulo="RESULTADO DO PERÍODO" detalhe="Receita líquida menos despesas" valor={moeda(resultado)} cor={resultado >= 0 ? "text-blue-700" : "text-red-700"} />
              <LinhaResumo titulo="MARGEM DO RESULTADO" detalhe="Resultado dividido pela receita líquida" valor={`${margemResultado.toFixed(2)}%`} cor={margemResultado >= 0 ? "text-green-700" : "text-red-700"} />
            </tbody>
          </table>
        </div>
          ...
          <table className="w-full border border-slate-300 text-xs">
            ...
            <tbody>
              {vendas.slice(0, 10).map((venda, index) => (
                ...
                >
                  <td className="p-3">{formatarDataHora(venda.created_at)}</td>
                  <td className="p-3">{venda.status || "-"}</td>
                  <td className="p-3 text-right text-red-700 font-bold">
                    {moeda(Number(venda.desconto || 0))}
                  </td>
