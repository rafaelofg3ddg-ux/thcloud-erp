...
export default function Sidebar() {
  ...
  function pode(modulo: string) {
    ...
    if (perfil === "Estoquista") {
      return ["dashboard", "produtos", "estoque", "relatorios"].includes(modulo);
    }

    if (perfil === "Vendedor") {
      return ["dashboard", "clientes", "vendas", "relatorios"].includes(modulo);
    }

    return false;
  }

  function moduloEmpresa(modulo: string) {
    if (isSuperAdmin) return true;
    if (modulo === "fiscal") return usuario?.modulo_fiscal === true;
    if (modulo === "whatsapp") return usuario?.modulo_whatsapp === true;
    if (modulo === "delivery") return usuario?.modulo_delivery === true;
    if (modulo === "crm") return usuario?.modulo_crm === true;
    if (modulo === "relatorios_premium") return usuario?.modulo_relatorios_premium === true;
    if (modulo === "multiloja") return usuario?.modulo_multiloja === true;
    return false;
  }
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 min-h-screen shadow-sm flex flex-col">
      ...
      <nav className="p-3 flex-1 overflow-y-auto">
        {isSuperAdmin && (
          <Link href="/admin" className="flex items-center gap-3 p-3 rounded-xl text-slate-700 hover:bg-blue-50 hover:text-blue-700 font-semibold transition">
            <Home size={19} /> Dashboard SaaS
          </Link>
        )}

        {pode("dashboard") && (
          <Link href="/dashboard" className="flex items-center gap-3 p-3 rounded-xl text-slate-700 hover:bg-blue-50 hover:text-blue-700 font-semibold transition">
          ...
          <div className="mt-1">
            ...
            {cadastros && (
              <div className="ml-8 mt-1 space-y-1 text-sm text-slate-500">
                {pode("produtos") && (
                  <>
                    <Link href="/produtos" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">
                      Produtos
                    </Link>

                    <Link href="/grupos" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">
                      Grupos
                    </Link>
                  </>
                )}
                ...
                {pode("fornecedores") && (
                  <Link href="/fornecedores" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">
                    Fornecedores
                  </Link>
                )}

                {pode("clientes") && (
                  <Link href="/clientes" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">
                    Clientes
                  </Link>
                )}
              </div>
        ...
        {pode("estoque") && (
          <div className="mt-1">
            ...
            {estoque && (
              <div className="ml-8 mt-1 space-y-1 text-sm text-slate-500">
                <Link href="/estoque/entrada" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">
                  Entrada
                </Link>

                <Link href="/estoque/saida" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">
                  Saída
                </Link>

                <Link href="/estoque/inventario" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">
                  Inventário
                </Link>
              </div>
            )}
        ...
        {pode("vendas") && (
          <div className="mt-1">
            ...
            {vendas && (
              <div className="ml-8 mt-1 space-y-1 text-sm text-slate-500">
                <Link href="/caixa/pdv" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">
                  PDV
                </Link>

                <Link href="/vendas/orcamentos" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">
                  Orçamentos
                </Link>

                <Link href="/vendas/pedidos" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">
                  Pedidos
                </Link>
              </div>
            )}
        ...
        {moduloEmpresa("delivery") && (
          <Link href="/delivery" className="flex items-center gap-3 p-3 rounded-xl text-slate-700 hover:bg-blue-50 hover:text-blue-700 font-semibold transition">
          ...
          <div className="mt-1">
            ...
            {relatorios && (
              <div className="ml-8 mt-1 space-y-1 text-sm text-slate-500">
                ...
                <Link href="/relatorios/dre" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">DRE</Link>
                <Link href="/relatorios/fluxo-caixa" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Fluxo de Caixa</Link>
                <Link href="/relatorios/contas-receber" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Contas a Receber</Link>
                <Link href="/relatorios/contas-pagar" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Contas a Pagar</Link>
                <Link href="/relatorios/produtos" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Produtos</Link>
                <Link href="/relatorios/estoque" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Estoque</Link>
                <Link href="/relatorios/clientes" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Clientes</Link>
                <Link href="/relatorios/produtos-mais-vendidos" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Produtos Mais Vendidos</Link>
                <Link href="/relatorios/ranking-clientes" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Ranking de Clientes</Link>
                <Link href="/relatorios/lucratividade" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Lucratividade</Link>
                <Link href="/relatorios/vendas" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Vendas por Período</Link>
                <Link href="/relatorios/inadimplentes" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Clientes Inadimplentes</Link>
                <Link href="/relatorios/compras-fornecedor" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Compras por Fornecedor</Link>
                <Link href="/relatorios/formas-pagamento" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Formas de Pagamento</Link>
                <Link href="/relatorios/curva-abc" className="block p-2 rounded-lg hover:bg-slate-100 hover:text-blue-700">Curva ABC</Link>

                {moduloEmpresa("relatorios_premium") && (
                  <>
      ...
      <div className="p-4 border-t border-slate-100">
        <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
          ...
          <button onClick={sair} className="mt-4 w-full flex items-center justify-center gap-2 bg-red-50 hover:bg-red-100 text-red-700 py-2 rounded-xl font-semibold">
            <LogOut size={16} /> Sair
          </button>
        </div>
      </div>
